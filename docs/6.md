...
根据优势函数（10）的定义，我们有 $Q^{\pi}(s,a)=A^{\pi}(s,a)+V^{\pi}(s)$，以及 $\mathbb{E}_{a\sim\pi(s)}[A^{\pi}(s,a)]=0$。另外，对于确定性策略（deterministic policy）（常用于基于值的深度 RL），$a*=\mathop{\arg\max} _{a'\in A}Q(s,a')$，由于$Q(s,a*)=V(s)$，我们有 $A(s,a*)=0$，这种情况下，贪婪动作的优势函数为零。
