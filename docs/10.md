...
**练习 6.5** 在证明以状态为变量的基准函数无偏的推导中，我们利用了 $\mathbb{E}_{a_t}[\nabla_{\theta} \log\pi_{\theta}(a_t|s_t)]=0$ 这一事实。如何证明这个期望为 $0$？
...
${\tt negative_likelihoods = tf.nn.softmax_cross_entropy_with_logits(labels=actions, logits=logits)}$