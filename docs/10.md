...
<span id="jump512">**引理 5.1**</span> 对于任意函数 $f:S\mapsto\mathbb{E}$ 和任意策略 $\pi$，我们有：
...
这一引理的证明可以参见 [4] 以及[附录 A.1](#jump511)。
...
注意，$\lVert d^{\pi'}-d^{\pi} \rVert_1$ 的上界已经由[引理 5.2](#jump522) 给出，因此可以代入式（6）和式（7）。
...
### <span id="jump511">A.1 奖励调整（Reward Shaping）</span>

这里我们证明[引理 5.1](#jump512)。
...
### <span id="jump521">A.2 状态分布差异限制（Bounding Difference in State Distributions）</span>

这里我们证明[引理 5.2](#jump522)。